...
export const sanitizeDecimals = (value, decimals = 18) => {
...
export const toWad = (amount, decimals = 18) => {
  return utils.parseUnits(sanitizeDecimals(amount, decimals), decimals);
};

export const fromWad = (wad, decimals = 18) => {
...
export function setLocaleStorageTestnetFlag(value) {
  if (typeof window !== "undefined") {
    window.localStorage.setItem(LOCALSTORAGE_KEY_TESTNET, `${value}`);
  }
}

export function getLocalStorageTestnetFlag() {
...
export const getProviderUrl = (chainId) => {
  return `https://rpc.walletconnect.com/v1/?chainId=${chainId}&projectId=${DEFAULT_PROJECT_ID}`;
};
...
export const getEthereumActions = ({
  openRequestModal,
  ethereumRpc,
  session,
}) => {
  const actions = {
    [DEFAULT_EIP155_METHODS.ETH_SEND_TRANSACTION]: {
      ...
      callback: async (
        ...
        openRequestModal?.();
        logger.info("[utilities,getEthereumActions]", "value", value);
        await ethereumRpc.testSendTransaction(
          chainId,
          address,
          Number(value),
          token,
    ...
    [DEFAULT_EIP155_METHODS.PERSONAL_SIGN]: {
      ...
      callback: async (chainId, address) => {
        openRequestModal?.();
...
export const getTronActions = ({
  ...
  const onSignTransaction = async (
    ...
    openRequestModal?.();
    if (value) {
      await tronRpc.testSignTransaction(
        chainId,
        address,
        value,
        token,
